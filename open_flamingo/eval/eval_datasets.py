...
class ExtendedVQADataset(Dataset):
    def __init__(
        self, image_dir_path, question_path, annotations_path, is_train, dataset_name
    ):
        self.questions = json.load(open(question_path, "r"))["questions"]
        ...
        # For VQAv2 and OK-VQA, verify the dataset format
        # if self.dataset_name in {"vqav2", "ok_vqa"}:
        #     self.img_coco_split = self.image_dir_path.strip("/").split("/")[-1]
        #     assert self.img_coco_split in {"train2014", "val2014", "test2015"}
    ...
    def get_img_paths(self, question):
        """
        Returns a list of image paths instead of a single image path.
        """
        # if self.dataset_name in {"vqav2", "ok_vqa"}:
        #     return [
        #         os.path.join(
        #             self.image_dir_path,
        #             f"COCO_{self.img_coco_split}_{image_id:012d}.jpg"
        #         ) for image_id in question["image_ids"]  # Now handling multiple image IDs
        #     ]
        # elif self.dataset_name == "vizwiz":
        #     return [os.path.join(self.image_dir_path, image_id) for image_id in question["image_ids"]]
        # elif self.dataset_name == "textvqa":
        #     return [os.path.join(self.image_dir_path, f"{image_id}.jpg") for image_id in question["image_ids"]]
    ...
    def __getitem__(self, idx):
        """
        Returns a list of images, along with the corresponding question and answer.
        """
        question = self.questions[idx]
        img_paths = self.get_img_paths(question)  # Get multiple image paths

        # Load multiple images instead of a single one
