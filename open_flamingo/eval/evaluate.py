        ...
        for shot in args.shots:
            ...
            for seed, trial in zip(args.trial_seeds, range(args.num_trials)):
                textvqa_score = evaluate_vqa(
                    args=args,
                    eval_model=eval_model,
                    num_shots=fixed_shot, # initially num_shots=shot
                    seed=seed,
...
def evaluate_vqa(
    ...
    train_dataset = ExtendedVQADataset(
        image_dir_path=train_image_dir_path,
        question_path=train_questions_json_path,
        annotations_path=train_annotations_json_path,
        is_train=True,
    ...
    test_dataset = ExtendedVQADataset(
        image_dir_path=test_image_dir_path,
        question_path=test_questions_json_path,
        annotations_path=test_annotations_json_path,
        is_train=False,
    ...
    for batch in tqdm(
        ...
        if args.rices:
            # print("\n\n\n====================> RICES MODE ON\n\n\n")
            batch_demo_samples = rices_dataset.find(batch["images"], effective_num_shots)
        else:
            # print("\n\n\n====================> RICES MODE OFF =====================\n\n\n")
            batch_demo_samples = utils.sample_batch_demos_from_query_set(
                query_set, effective_num_shots, len(batch["images"])
            )
        ...
        for i in range(len(batch["images"])):
            if num_shots > 0:
                # print("\n\n==> FEW SHOT MODE ON")
                context_images = [x["images"] for x in batch_demo_samples[i]]
            else:
                # print("\n\n==> ZERO SHOT MODE ON")
                context_images = []
                
            # print("\n\n\nImage len: ", len(batch["images"][i]))
            # print("\n\n\nImage for testing: ", batch["images"][i])    
            
            # batch_images.append(context_images + [batch["images"][i]])
            batch_images.append(context_images + batch["images"][i])
            ...
            # batch_text.append(
            #     context_text + "<image><image>" + eval_model.get_vqa_prompt(question=batch["question"][i])
            # )
            
            # print("Eval_model: ", eval_model)
            # print("Question: ", eval_model.get_vqa_prompt(question=batch["question"][i]))
            
            batch_text.append(
                "<image>" *  (len(batch["images"][i])) + eval_model.get_vqa_prompt(question=batch["question"][i])
            )

        # print("\n\n\n===> Batch image length: ", len(batch_images[0]))
        
        # print("\n\nbatch images: ", batch_images[0])
